...
function getAIRecommendations(totalCost, materialCost, laborCost, workType, area) {
    const data = {
        totalCost: totalCost,
    ...
    .then(data => {
        if (data.success) {
            document.getElementById('ai-recommendation-text').innerText = data.recommendation;
        } else {
            console.error('Ошибка при получении рекомендаций:', data.error);
        }
    })
    .catch(error => {
